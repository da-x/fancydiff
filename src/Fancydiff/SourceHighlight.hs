...
nullMatcher,
  haskellMatcher,
  clangMatcher,
  javascriptMatcher :: Text -> Either String FList
...
proceduralMatcher :: Int -> Text -> Either String FList
proceduralMatcher startCode = parseWithAlex startCode (\x -> runST $ p x)
    where
        p lst = do out <- ST.newSTRef DList.empty
